...
```console
$ pytest tests/calculations/test_hp.py
```
...
```console
$ uv run pytest tests/calculations/test_hp.py
```
...
```console
$ hatch test tests/calculations/test_hp.py
```
...
In the following, we assume that the latest release was `v0.6.0`.
Furthermore, we assume the following naming conventions for remotes:

* `origin`: remote pointing to `aiidateam/aiida-hubbard`
...
* Patch release: `v0.6.0` to `v0.6.1`, only bug fixes
* Minor release: `v0.6.0` to `v0.7.0`, bug fixes and new features that **maintain** backwards compatibility
* Major release: `v0.6.0` to `v1.0.0`, bug fixes and new features that **break** backwards compatibility

#### Creating the release branch

We use the [GitHub Flow](https://www.flagship.io/git-branching-strategies/) branching model. In short: new features, bug fixes, etc are added by opening a pull request to `master`, and the `master` branch is tagged after doing a pull request that updates the `CHANGELOG.md`. Doing so will trigger an automated deployment to PyPI.

For most releases, we assume that all the changes in the current `master` branch have to be included in the release. As such, branch off the new release branch directly from `master`:

    git fetch --all --prune
    git checkout origin/master -b release/0.7.0
...
Once you've prepared the release branch locally, commit with the message 'Release `v0.7.0`' and push it to Github. For our major/minor release example:
...
Now that the release branch is ready, merge it into `master` via a pull request.
Make sure the remote `release/0.7.0` branch is up to date by pushing the local changes, then go to Github and create a pull request to the `master` branch of the official repository.

After the pull request has been approved, merge the PR using the "Squash and Merge", and make sure the commit is simply named e.g. 'Release `v0.7.0`'.

Once this is complete, fetch all the changes locally, checkout the `master` branch and make sure it's up to date with the remote:
...
Double check that the tag is correct, and that the `master` branch looks in good shape.
:::
...
Once you're confident that the tag and `master` branch are in good shape, push both to the remote:
...
:::{important}
In case you did push with the wrong tag, the CD should fail since we [validate the tag](https://github.com/aiidateam/aiida-hubbard/blob/master/.github/workflows/validate_release_tag.py) as part of the workflow.
